...
const {transformOrder, transformOrderDetail} = require('./merge');
...
module.exports = {
    ...
    createOrder: async (args, { req }) => {
        ...
        try {
            ...
            for(let i=0; i<args.orderInput.device.split(",").length; i++) {
                let orderDetail = new OrderDetail({
                    orderId: order._id,
                    deviceTypeId: args.orderInput.device.split(",")[i].split(":")[0],
                    quantity: args.orderInput.device.split(",")[i].split(":")[1]
                });
